...
function dottedToNumber(dotted) {
  ...
  return (((((Number(splitted[0]) * 256) + Number(splitted[1])) * 256) + Number(splitted[2])) * 256) + Number(splitted[
    3]);
}
...
function List(url, prefix) {
  ...
  this.regex = new RegExp('^' + this.prefix +
    '((?:(?:25[0-5]|2[0-4][0-9]|1[0-9]{2}|[1-9][0-9]|[0-9])\\.){3}(?:25[0-5]|2[0-4][0-9]|1[0-9]{2}|[1-9][0-9]|[0-9])(?:/(?:3[0-2]|[1-2][0-9]|[0-9]))?)'
  );
}
/**
    ...
    async.parallel([
      ...
      if (err) {
        ...
        ipSets.forEach(function(ipSet, index) {
          ...
          console.log('IP Set ' + ipSetName + ' has ' + ipSetDescriptors.length +
            ' descriptors and should have ' + rangeSlice.length);
          var updates = [];
              ...
              return function(callback) {
                async.waterfall([
                  ...
                  function(response, callback) {
                    console.log('Updating IP set ' + ipSetName + ' with ' + updateBatch.length +
                      ' updates');
                    waf.updateIPSet({
        ...
        if (tasks.length > 0) {
          ...
          async.series(tasks, function(err) {
            ...
            done(context, err, err ? 'Error updating IP sets' : 'Updated IP sets' + (notFitCount > 0 ? ', ' +
              notFitCount + ' ranges unable to fit in IP sets' : ''));
          });
        } else {
